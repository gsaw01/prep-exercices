...
for (let passwordItem of passwordList) {
    
    let [min, max] = passwordItem.times.split('-').map(Number);
    let targetLetter = passwordItem.letter;
    let passwordArr = passwordItem.password.split('');

    let targetLetterCount = passwordArr.filter(letter => letter === targetLetter).length;
    
    if (targetLetterCount >= min && targetLetterCount <= max) {
        console.log(`'${passwordItem.password}' is VALID, ${passwordItem.letter} is present ${targetLetterCount} times and should have been present at least ${min} times and at most ${max} times`);
    } else {
        console.log(`'${passwordItem.password}' is INVALID, ${passwordItem.letter} is present ${targetLetterCount} times and should have been present at least ${min} times and at most ${max} times`);
    }
}